...
raw = JSON.parse(fs.read-file-sync 'gradients.json' .toString!)
gradients = {}
if Array.isArray(raw) =>
  for i from 0 til raw.length => gradients[(i + 1)] = raw[i]
else gradients <<< raw
...
gradient = (req, res) ->
  data = JSON.parse(decodeURIComponent(req.queries.data or ''))
  if data.id? =>
    if req.method == \DELETE => delete gradients[data.id]
    else if req.method == \GET => gradients[data.id] = data
  fs.write-file-sync "gradients.json", JSON.stringify(gradients)
  res.writeHead 200 {"Content-type": "text/html"}
  res.end ""

gradient-order = (req, res) ->
  data = decodeURIComponent(req.queries.data or '').split(',')
  fs.write-file-sync "orders.json", JSON.stringify(data)
  res.writeHead 200 {"Content-type": "text/html"}
  res.end ""


route-table = do
  "/sample-cgi": sample-cgi
  "/gradient": gradient
  "/gradient/order": gradient-order

server = (req, res) ->
  req.query = /\?([^#]+)/.exec(req.url)
  req.queries = {}
  if req.query =>
    req.query.1
      .split \&
      .map -> it.split \=
      .map -> req.queries[it.0] = [it.1]
  req.url = req.url - /[?#].*$/
...
http.createServer server .listen 9876, \0.0.0.0

console.log "running server on 0.0.0.0:9876"
