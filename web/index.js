...
(function(){
  var colors, getColorAlt, CENS, SA, local, GradientEditor, node, ldcp;
  ...
  window.GradientEditor = GradientEditor = function(opt){
    ...
    this.root = root = typeof opt.root === typeof ''
      ? opt.root = document.querySelector(opt.root)
      : opt.root;
    this.root.classList.add('ldgradient');
    this.id = opt.id || "ldg-" + Math.random().toString(36).substring(2);
    this.dir = opt.dir || 0;
    this.angle = {};
    this.state = {
      drag: false,
    ...
    if (!opt.colors || opt.colors.length < 2) {
      opt.colors = Array.from(this.root.querySelectorAll('g.ldg-colors circle')).map(function(it){
    ...
    this.el = {
      bar: this.root.querySelector('.ldg-bar'),
      colors: this.root.querySelector('.ldg-colors'),
      board: this.root.querySelector('.ldg-board'),
      gradient: gradient = this.root.querySelector('linearGradient'),
      texts: this.root.querySelector('.ldg-texts'),
      percent: this.root.querySelector('.ldg-percent'),
      hex: this.root.querySelector('.ldg-hex')
    };
    this.setIdx(0);
    this.buildDots();
    this.updateGradient();
  ...
  GradientEditor.prototype = import$(Object.create(Object.prototype), {
    ...
    getDots: function(){
      return Array.from(this.el.colors.querySelectorAll('g'));
    },
